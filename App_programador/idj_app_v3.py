...
class App(tk.Tk):
    def __init__(self):
        ...
        self.bind('<Escape>', lambda e: None)  # deshabilita Escape para evitar salidas accidentales
        ...
        self.var_status    = tk.StringVar(
            value="Presiona ESCANEAR para buscar el IDJ Programador")
    ...
    # =========================================================================
    # PANTALLA 1 — CONEXIÓN  (sin cambios)
    # =========================================================================
    ...
    def _build_prog(self, p):
        ...
        # Numpad: BW=192, BH=128, GAP=10 → total_w=596, total_h=542
        NP_BW, NP_BH, NP_GAP = 192, 128, 10
        NP_W = 3 * NP_BW + 2 * NP_GAP   # 596
        NP_H = 4 * NP_BH + 3 * NP_GAP   # 542

        # El numpad se coloca pegado a la derecha con 15px de margen
        NP_RIGHT_MARGIN = 15
        NP_X = W - NP_W - NP_RIGHT_MARGIN  # 669

        # El formulario ocupa desde x=10 hasta 20px antes del numpad
        FORM_MARGIN = 10
        FX    = 10                          # x inicio formulario
        FW    = NP_X - FX - FORM_MARGIN     # ancho formulario ≈ 644

        # Separador vertical justo entre formulario y numpad
        SEP_X = NP_X - FORM_MARGIN // 2
        ...
        st_card.place(x=FX, y=body_y + 8, width=FW, height=210)

        tk.Label(st_card, text="ESCLAVO CONECTADO — ESTADO ACTUAL",
                 bg=PANEL, fg=GRAY2,
                 font=("Arial", 10, "bold")).place(x=12, y=8)

        card_w = (FW - 34) // 2   # ancho de cada tarjeta jaula/dolly

        # Jaula actual
        jcard = tk.Frame(st_card, bg=CARD)
        jcard.place(x=10, y=28, width=card_w, height=118)
        ...
        tk.Label(jcard, textvariable=self.var_cur_jaula,
                 bg=CARD, fg=GREEN,
                 font=("Arial", 24, "bold"),
                 wraplength=card_w - 20).place(x=12, y=40)

        # Dolly actual
        dcard = tk.Frame(st_card, bg=CARD)
        dcard.place(x=card_w + 22, y=28, width=card_w, height=118)
        ...
        # Botón LEER — ancho completo
        self._btn(st_card, "↻   LEER EEPROM",
                  BLUE, self._on_leer,
                  x=10, y=154, w=FW - 20, h=48,
                  font=("Arial", 14, "bold"))

        # Separador horizontal
        tk.Frame(p, bg=CARD, height=2).place(
            x=FX, y=body_y + 226, width=FW, height=2)

        # ── Formulario ────────────────────────────────────────────────────────
        fy = body_y + 238
        FIELD_W = FW - 8   # ancho de los campos de entrada

        tk.Label(p, text="PROGRAMAR NUEVO NÚMERO",
                 bg=BG, fg=GRAY2,
                 font=("Arial", 11, "bold")).place(x=FX + 8, y=fy)

        # Campo Jaula
        tk.Label(p, text="Número de Jaula  (T0603-xxxx)  *requerido",
                 bg=BG, fg=GRAY2,
                 font=("Arial", 11)).place(x=FX + 8, y=fy + 24)

        self.fr_jaula = tk.Frame(p, bg=BLUE, padx=3, pady=3)
        self.fr_jaula.place(x=FX + 4, y=fy + 45, width=FIELD_W, height=62)
        self.lbl_j = tk.Label(self.fr_jaula,
                              textvariable=self.var_jaula,
                              bg=CARD, fg=WHITE,
                              font=("Arial", 28, "bold"),
                              anchor="e", padx=14, cursor="hand2")
        ...
        tk.Label(p, text="Número de Dolly  (T0605-xxxx)  — 0 si no aplica",
                 bg=BG, fg=GRAY2,
                 font=("Arial", 11)).place(x=FX + 8, y=fy + 120)

        self.fr_dolly = tk.Frame(p, bg=GRAY, padx=3, pady=3)
        self.fr_dolly.place(x=FX + 4, y=fy + 141, width=FIELD_W, height=62)
        ...
        self.lbl_act = tk.Label(p,
                                ...
                                bg=BG, fg=GRAY, font=("Arial", 11))
        self.lbl_act.place(x=FX + 8, y=fy + 215)

        # Botones — PROGRAMAR ocupa ~65%, LIMPIAR el resto
        BTN_Y  = fy + 238
        BTN_H  = 64
        PROG_W = int(FIELD_W * 0.64)
        LIMP_W = FIELD_W - PROG_W - 12

        self.btn_prog = self._btn(p, "▶  PROGRAMAR",
                                  ORANGE, self._on_programar,
                                  x=FX + 4, y=BTN_Y, w=PROG_W, h=BTN_H,
                                  font=("Arial", 16, "bold"))

        self._btn(p, "✕  LIMPIAR",
                  CARD, self._on_limpiar,
                  x=FX + 4 + PROG_W + 12, y=BTN_Y, w=LIMP_W, h=BTN_H,
                  font=("Arial", 13, "bold"))

        # ── Separador vertical ────────────────────────────────────────────────
        tk.Frame(p, bg=CARD, width=2).place(
            x=SEP_X, y=body_y, width=2, height=body_h)

        # ── Numpad centrado en su sección ─────────────────────────────────────
        NP_OX = NP_X
        NP_OY = body_y + (body_h - NP_H) // 2 + 10

        tk.Label(p, text="TECLADO NUMÉRICO",
                 bg=BG, fg=GRAY2,
                 font=("Arial", 12, "bold")).place(
                     x=NP_OX, y=NP_OY - 32, width=NP_W)
        ...
        for (txt, row, col, color) in keys:
            bx = NP_OX + col * (NP_BW + NP_GAP)
            by = NP_OY + row * (NP_BH + NP_GAP)
            if txt == "←":   cmd = self._on_back
            elif txt == "✓": cmd = self._on_ok
            else:             cmd = lambda t=txt: self._on_digit(t)
            tk.Button(p, text=txt,
                      bg=color, fg=WHITE,
                      font=("Arial", 28, "bold"),
                      relief="flat",
                      activebackground=BLUE, activeforeground=WHITE,
                      command=cmd).place(x=bx, y=by, width=NP_BW, height=NP_BH)
    ...
    def _on_ok(self):
        if self.active == "jaula": self._set_active("dolly")
        else:                      self._on_programar()
